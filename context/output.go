...
//输出结构体定义
type ThingoOuput struct {
	Context *ThingoContext //本次请求相关的上下文
...
//获取一个输出实例
func NewThingoOutput() *ThingoOuput {
	output := &ThingoOuput{}
	return output
}

//重置输出信息
func (output *ThingoOuput) Reset(ctx *ThingoContext) {
	output.Context = ctx
	output.Status = 0
	output.Body = []byte{}
...
//设置要输出的header信息
func (output *ThingoOuput) AddHeader(key, val string) {
	output.Context.ResponseWriter.Header().Set(key, val)
}

//设置输出的body
func (output *ThingoOuput) SetBody(body []byte) {
	output.Body = body
}
...
*/
func (output *ThingoOuput) AddCookie(name string, value string, others ...interface{}) {
	var b bytes.Buffer
...
//返回json数据
func (output *ThingoOuput) RenderJson(data interface{}) error {
...
func (output *ThingoOuput) RenderJsonp(data interface{}, callback ...string) error {
	var content []byte
	var err error
	content, err = json.Marshal(data)
	if err != nil {
...
//设置响应的状态值
func (output *ThingoOuput) SetStatus(status int) {
	output.Status = status
}

//输出所有的内容，包括header、body、cookie等
func (output *ThingoOuput) Send() {
	//输出cookie信息
